...
## 먼저 이것부터 — Gitea가 GitHub를 직접 끌어올 수 있는가

**되면 파일을 옮길 일이 아예 없습니다.** 2분이면 되는지 아닌지 알 수 있으니
다른 것을 준비하기 전에 이것부터 해 보세요.

Gitea에는 남의 저장소를 통째로 가져오는 기능이 있습니다. 그리고 이 저장소는
**공개**라 토큰이 필요 없습니다 — 체크리스트의 P2(GitHub 배포 토큰)가 통째로
사라집니다.

### 사내 PC 브라우저에서, Gitea에 로그인한 뒤

1. 오른쪽 위 **+** → **새 마이그레이션**(New Migration)
2. **GitHub** 선택 (없으면 **Git** 도 됩니다)
3. **복제 주소**: `https://github.com/6juuuuuun/weather-management.git`
4. **인증은 비워 둡니다** — 공개 저장소입니다
5. **저장소 이름**: `weather-management`
6. ☑ **이 저장소를 미러로 만듭니다**(This repository will be a mirror) ← 권장, 이유는 아래
7. **마이그레이션** 클릭

**성공하면** 커밋 이력이 그대로 보입니다. 서버에서 바로 받으면 됩니다:
...
**실패하면**(`Could not resolve host`·시간 초과 등) Gitea도 밖으로 못 나가는
것이니 아래 B-1·B-2로 갑니다. 이 시도 자체가 검사입니다.

### 미러로 만들면 무엇이 좋은가

- **미러**: Gitea가 GitHub를 주기적으로 스스로 따라옵니다. 개발 담당자가 GitHub에
  푸시하면, 잠시 뒤 사내 Gitea에도 반영되고 서버는 `git pull`만 하면 됩니다.
  사람이 중간에서 옮기는 단계가 **영구히 사라집니다.**
- **일반 마이그레이션**(미러 체크 해제): 지금 시점의 복사본 한 벌입니다. 이후
  갱신은 누군가 손으로 밀어 넣어야 합니다.

미러 저장소는 Gitea에서 **읽기 전용**입니다. 원본이 GitHub이므로 그게 맞습니다.
Gitea 저장소 설정에서 동기화 주기를 줄이거나 **지금 동기화**를 눌러 즉시 당길 수 있습니다.

> 나중에 GitHub 저장소를 비공개로 바꾸면, 이 미러에 읽기 전용 토큰을 넣어 줘야
> 계속 따라옵니다(Gitea 저장소 설정 → 미러 설정 → 인증).

### ⚠️ 이것이 되어도 아직 안 끝났습니다

Gitea가 GitHub를 끌어오는 것과, **Docker가 Docker Hub·npm에 나가는 것은 다른
문제**입니다. 코드가 들어와도 서버가 이미지를 못 받으면 빌드가 멈춥니다.
아래 검사를 서버에서 꼭 해 보고, `dockerhub`·`npm`이 `000`이면 **B-2**의
이미지 반입이 여전히 필요합니다.

---

## 무엇이 막혔는지에 따라 길이 갈립니다

§2의 세 줄 결과를 봅니다.

| Gitea가 GitHub를 끌어옴 | dockerhub · npm | 가는 길 |
|---|---|---|
| ✅ | ✅ | **위 미러로 끝.** 파일을 옮길 일이 없습니다 |
| ✅ | ❌ | 코드는 미러로, **이미지만 B-2**로 반입 |
