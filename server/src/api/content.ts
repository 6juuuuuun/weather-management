...
// staff_actions와 guest_notice는 **특보 문자 본문에 그대로 실린다**. 상한이
// 없던 동안 5000자짜리 값이 그대로 저장됐고, 그런 지침 하나가 붙은 특보는
// 승인 화면과 문자 양쪽을 못 읽는 상태로 만든다.
//
// **여기의 상한은 LMS 한 통(2,000바이트)을 보장하지 않는다.** 20개 × 200자 +
// 1,000자는 한글로 15,000바이트가 넘는다. 한 항목이 너무 길어 화면이 깨지는 것을
// 막는 값이고, "한 통에 담기는가"는 항목 하나가 아니라 **부서별 지침 전체의 합**이
// 정하는 다른 질문이다. 그쪽은 워치독이 미리 재고(jobs/watchdog.ts) 발송 시점에는
// 잘림 표시가 붙는다(shared/sms.ts의 fitToLms) — 여기서 합계까지 막지 않는 이유는
// 지침이 부서·종류·등급별로 따로 저장돼, 저장 시점에는 어떤 조합이 한 통이 될지
// 알 수 없기 때문이다. 글자 수는 코드 포인트로 센다
